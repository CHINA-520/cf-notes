export default {
  async fetch(request, env) {
    ...
    // 新增/修改手记接口
    if (pathname === "/note" && (request.method === "POST" || request.method === "PUT")) {
      const body = await request.json();
      // 如果是 PUT 则使用传入的 id，否则生成新 id
...
function renderIndexPage() {
  ...
  async function loadNotes(){
    ...
    notesData.forEach(n=>{
      ...
      card.innerHTML=\`
        ...
        <div style="font-size:0.95rem;color:#8a7a68;margin-bottom:1rem">\${n.content.slice(0,50)}...</div>
        <div style="display:flex;gap:0.5rem">
          <button class="action-btn" onclick="location.href='/?id=\${n.id}'">查看</button>
          <button class="action-btn" onclick="editNote('\${n.id}')">编辑</button>
          <button class="action-btn" onclick="deleteNote('\${n.id}')">删除</button>
        </div>\`;
  ...
  async function editNote(id){
    const res = await fetch(location.origin+"/note/"+id);
    if(res.ok){
        const n = await res.json();
        editingId = id;
        editingCreatedAt = n.createdAt;
        titleInput.value = n.title;
        tagsInput.value = (n.tags || []).join(", ");
        contentInput.value = n.content;
        formTitle.textContent = "修 改 手 记";
  ...
  saveBtn.onclick=async()=>{
    ...
    if(editingId) {
        ...
        payload.createdAt = editingCreatedAt;
    }

    await fetch(location.origin+"/note",{
        method: method,
